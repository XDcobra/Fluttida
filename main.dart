...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Scenario 7: WebView headless (iOS: WKWebView loadRequest)
  // ---------------------------------------------------------------------------
  Future<RequestResult> _requestWebViewHeadless(RequestConfig cfg) async {
    final finished = Completer<void>();
    final error = Completer<String?>();

    // Controller einmalig erstellen und im Widget-Tree behalten
    _headlessWebViewController ??= WebViewController()
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onWebResourceError: (err) {
            final msg = "WV error: ${err.errorCode} ${err.description}";
            debugPrint(msg);
            if (!error.isCompleted) error.complete(msg);
          },
    ...
    // Wichtig: wenn du _headlessWebViewController erst jetzt setzt, setState, damit Offstage es rendert
    setState(() {});

    final sw = Stopwatch()..start();

    // loadRequest unterstützt headers + method; body nur bei POST (plugin-abhängig)
    await _headlessWebViewController!.loadRequest(
      Uri.parse(cfg.url),
    ...
    final done = await Future.any([
      ...
      error.future,
      Future.delayed(cfg.timeout, () => 'timeout'),
    ]);
    ...
    if (done != null) {
      return RequestResult(
        ...
        error: 'WebView load failed: $done',
      );
    }

    final htmlAny = await _headlessWebViewController!
        .runJavaScriptReturningResult('document.documentElement.outerHTML');

    return RequestResult(
      status: null, // WKWebView gibt dir keinen HTTP status “einfach so”
      body: htmlAny.toString(),
      durationMs: sw.elapsedMilliseconds,
    );
  }
